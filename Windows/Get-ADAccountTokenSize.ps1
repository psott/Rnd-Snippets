function Get-ADAccountTokenSize {
  param (
  ...
  $ExportPath = 'C:\Temp\TokenSize'
  
  if(!(Test-Path -Path C:\Temp)){
    $null = New-Item -Name Temp -Path C:\ -ItemType Directory
  }
  if(!(Test-Path -Path $ExportPath)){
    $null = New-Item -Name TokenSize -Path C:\Temp -ItemType Directory
  }
  
  if(!(Test-Path -Path "$ExportPath\GroupDetails")){
    $null = New-Item -Name GroupDetails -Path $ExportPath -ItemType Directory
  }
  if(!(Test-Path -Path "$ExportPath\GroupSIDHistoryDetails")){
    $null = New-Item -Name GroupSIDHistoryDetails -Path $ExportPath -ItemType Directory
  }
  if(!(Test-Path -Path "$ExportPath\UserSIDHistoryDetails")){
    $null = New-Item -Name UserSIDHistoryDetails -Path $ExportPath -ItemType Directory
  }
  ...
  foreach ($Principal in $Principals) { 
    ###null
    $ErrorStatus = $ErrorDetails = $UserNameShort = $Username = $PrincipalsDomain = $Tokensize = $DelegatedTokenSize = $null
    $Maxtokensizevalue = $ProblemDetected = $ProblemDetails = $GroupCount = $SIDHistoryCounter = $GroupSidHistoryCounter = $null
    $AllSIDHistories = $SecurityGlobalScope = $SecurityDomainLocalScope = $SecurityUniversalInternalScope = $SecurityUniversalExternalScope = $null
    $Groups = $UserIdentity = $null
    ###
    $i++
    Write-Host "User $i von $($Principals.Count)"
    try{
    $UserIdentity = New-Object System.Security.Principal.WindowsIdentity($Principal) -ErrorAction Stop
    ...
    if(!(Test-Path -Path "$ExportPath\GroupDetails\$PrincipalsDomain")){
      $null = New-Item -Name $PrincipalsDomain -Path "$ExportPath\GroupDetails" -ItemType Directory
    }
    if(!(Test-Path -Path "$ExportPath\GroupSIDHistoryDetails\$PrincipalsDomain")){
      $null = New-Item -Name $PrincipalsDomain -Path "$ExportPath\GroupSIDHistoryDetails" -ItemType Directory
    }
    if(!(Test-Path -Path "$ExportPath\UserSIDHistoryDetails\$PrincipalsDomain")){
      $null = New-Item -Name $PrincipalsDomain -Path "$ExportPath\UserSIDHistoryDetails" -ItemType Directory
    }
    ...
    $OutputTokenSize += [pscustomobject]@{
  ...
  $rnd = Get-Random
  $OutputTokenSize | Export-Csv -Path "$ExportPath\UserTokenSize-$rnd.csv" -UseCulture -NoTypeInformation -Encoding UTF8 -Append
}
